...
export default function OpenclawAndUs() {
  ...
  const topics: { title: string; author: string; year?: string; quote: string; link?: string; tags?: string[] }[] =
    t('ai-agent-and-us.topics') || []
    ...
    <>
      ...
      <div className="min-h-screen bg-[#fff9f5] wave-bg">
        <main className="relative z-10 min-h-screen flex flex-col items-center justify-center px-8 pt-32 pb-20">
          ...
          <div
            className={`w-full max-w-5xl ${mounted ? 'animate-fade-up' : 'opacity-0'}`}
                    ...
                    >
                      <div className="p-5">
                        <div className="flex items-start justify-between mb-3">
                          <h3
                            className={`text-lg font-bold ${colorScheme.accent} group-hover:translate-x-0.5 transition-transform duration-300 flex-1 pr-2`}
                          >
                            {topic.title}
                          </h3>
                          {topic.year && (
                        ...
                        <div className="text-sm text-[#636e72] mb-3 font-medium">{topic.author}</div>

                        <p className="text-[#2d3436] leading-relaxed text-sm mb-3">{topic.quote}</p>
                        ...
                        {topic.tags && topic.tags.length > 0 && (
                          <div className="flex flex-wrap gap-2 mb-3">
                            {topic.tags.map((tag, i) => (
                              <span
                                key={i}
                                className={`text-xs px-2.5 py-1 rounded-md bg-white border ${colorScheme.border} ${colorScheme.accent} font-medium shadow-sm`}
                              >
                                {tag}
                              </span>
                            ))}
